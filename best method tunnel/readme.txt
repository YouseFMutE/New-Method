راهنمای قدم‌به‌قدم (فارسی)
===========================

هدف: ساخت و اجرای یک تونل استاندارد و امن برای فوروارد کردن یک پورت مشخص
از Server به Client با یک باینری مشترک (CLI + منوی گرافیکی ترمینالی).

1) پیش‌نیازها
--------------
- سیستم‌عامل: Linux
- دسترسی root یا sudo برای باز کردن پورت
- داشتن Rust (برای Build) یا فایل باینری آماده

2) ساخت یا دریافت باینری
------------------------
الف) اگر سورس دارید:
   - نصب Rust
   - اجرای:
     cargo build --release

ب) اگر باینری آماده دارید:
   - فایل باینری را در مسیر دلخواه قرار دهید

2.1) نصب تک‌خطی (پیشنهادی)
---------------------------
روی هر سرور:
  bash <(curl -fsSL https://raw.githubusercontent.com/YouseFMutE/New-Method/main/best%20method%20tunnel/scripts/install.sh)

این نصب، پرسش‌نامه CLI را اجرا می‌کند تا نقش (Server/Client) و تنظیمات را وارد کنید.

3) اجرای اولیه (مرحله‌ای)
--------------------------
فرمان نمونه:
  mytunnel init

در این مرحله برنامه از شما می‌پرسد:
- نقش: Server یا Client
- نوع انتقال (Transport):
  - `app_tcp` (پیشنهادی): Handshake شبیه `HTTP CONNECT` و سپس کانال باینری پایدار
  - `raw`: حالت قدیمی باینری
- پورت عمومی روی Server (مثلا 1414)
- پورت تونل روی Server (مثلا 9000)
- IP مجاز برای تونل روی Server (اختیاری ولی برای پورت 443 شدیدا توصیه می‌شود)
- مقصد نهایی روی Client (مثلا 127.0.0.1:1414)
- تعداد اتصال‌های هم‌زمان تونل (TCPmux)
- کلید مشترک امنیتی (PSK)
  - امکان تولید PSK رندوم
  - امکان ورود دستی PSK
- زمان تلاش مجدد اتصال (reconnect)
  - reconnect_delay_ms و reconnect_max_delay_ms

پس از پاسخ‌ها، فایل تنظیمات ساخته می‌شود.

4) اجرای سرویس
--------------
Server:
  mytunnel run
  - پورت public_listen برای کاربران
  - پورت tunnel_listen برای اتصال Client

Client:
  mytunnel run

5) راه‌اندازی سیستم‌سرویسی (systemd)
-------------------------------------
اسکریپت آماده:
  sudo /PATH/TO/scripts/first_run.sh

این اسکریپت:
- اگر cargo نصب نباشد، Rust را نصب می‌کند
- پروژه را build می‌کند
- فایل config را در /etc/mytunnel/config.toml می‌سازد
- سرویس systemd می‌سازد و اجرا می‌کند
 - لاگ را در /var/log/mytunnel/mytunnel.log ذخیره می‌کند

در نصب جدید، **همیشه** پرسش‌نامه CLI اجرا می‌شود و از شما مرحله‌به‌مرحله تنظیمات را می‌گیرد.

6) تست سریع
------------
از یک کلاینت دیگر روی اینترنت یا شبکه:
  nc <server_ip> 1414
باید داده به مقصد روی Client برسد.

7) نکات امنیتی
--------------
- این سیستم از PSK برای احراز هویت **یک‌طرفه** استفاده می‌کند (Server فقط صحت را بررسی می‌کند).
- ارتباط **رمزنگاری ندارد** و داده‌ها **plaintext** هستند.
- PSK باید قوی باشد (حداقل 32 بایت تصادفی).
- PSK را در اختیار شخص ثالث قرار ندهید.
- پورت Server را در فایروال فقط برای IPهای مجاز باز کنید.
- اگر تونل روی `443` است، در `config.toml` سرور مقدار `tunnel_allow_ips` را حتما پر کنید
  تا اسکنرها باعث timeout/reset نشوند.

8) نکات کارایی
---------------
- این نسخه از TCPmux داخلی (frame-based) استفاده می‌کند و سربار کمی دارد.
- برای پایداری بهتر روی برخی مسیرها، `transport = "app_tcp"` را روی هر دو سرور یکسان بگذارید.
- اگر لینک ناپایدار است، در Client می‌توانید `mux_con` را بیشتر کنید (مثلاً 2 یا 3).

9) خطاهای رایج
---------------
- اتصال برقرار نمی‌شود:
  - پورت‌ها را چک کنید.
  - فایروال را بررسی کنید.
  - وضعیت سرویس را ببینید.

- کندی یا قطع و وصل:
  - کیفیت لینک را بررسی کنید.
  - محدودیت‌های شبکه را در نظر بگیرید.

پشتیبانی:
----------
اگر نیاز به امکانات اضافه (چند پورت، داشبورد، لاگ پیشرفته) دارید، اعلام کنید.

10) مانیتورینگ سریع
--------------------
اسکریپت مانیتورینگ:
  sudo /PATH/TO/scripts/monitor.sh

لاگ‌ها:
  tail -f /var/log/mytunnel/mytunnel.log

همچنین:
  systemctl status mytunnel
  journalctl -u mytunnel -f

11) دیباگ (اختیاری)
--------------------
برای لاگ بیشتر:
  MYTUNNEL_DEBUG=1 /usr/local/bin/mytunnel run --config /etc/mytunnel/config.toml
